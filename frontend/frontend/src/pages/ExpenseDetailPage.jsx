...
import { approveExpenseRecord, fetchExpenseRecord } from "../api/client";
  ...
  return (
    <div>
      ...
      <h2>
        ...
        {new Date(record.month).toLocaleDateString(undefined, { year: "numeric", month: "short" })}
      </h2>
      <p>Status: {record.status}</p>
      <p>Total amount: {record.total_amount}</p>

      <h3>Line items</h3>
      {record.line_items?.length === 0 && <p>No line items yet.</p>}
      ...
      <div className="actions">
        ...
        >
          {record.status === "approved" ? "Approved" : approving ? "Approving..." : "Approve"}
        </button>
      </div>
    </div>
  );
